...
public class LoadingUtil {
	...
	public static void importPreviousQueries(StandaloneSupport support, RequiredData content, User user) throws JSONException, IOException {
		...
		for (ResourceFile queryResults : content.getPreviousQueryResults()) {
			...
			String[][] data = CSV.streamContent(config.getCsv(), queryResults.stream()).toArray(String[][]::new);

			ConceptQuery q = new ConceptQuery();
			q.setRoot(new CQExternal(Arrays.asList(FormatColumn.ID, FormatColumn.DATE_SET), data));

			ManagedQuery managed = support.getNamespace().getQueryManager().createQuery(q, queryId, user);
