...
const formatDate = date => (date ? date.format(DATE_FORMAT) : null);
...
const QueryGroupModal = (props: PropsType) => {
  ...
  const onChangeRawMin = value => {
    ...
    if (!minDate) minDate = parseDatePattern(value);

    onSetMinDate(formatDate(minDate));

    if (maxDate && maxDate.isValid) onSetMaxDate(formatDate(maxDate));
  };

  const onChangeRawMax = value => {
    var { maxDate } = specificDatePattern(value);

    if (!maxDate) maxDate = parseDatePattern(value);
  ...
  return (
    <Modal closeModal={props.onCloseModal} doneButton tabIndex={3}>
      <div className="query-group-modal">
        <h3 className="query-group-modal__headline">
          {props.group.elements.reduce(
            (parts, concept, i, elements) => [
              ...parts,
              <span key={i} className="query-group-modal__headline-part">
        ...
        <div
          className={`query-group-modal__dates ${
            ie.version && ie.version === 11 ? " ie11" : ""
          }`}
        >
          <div className="query-group-modal__input-group">
            ...
            <DatePicker
              ...
              className={classnames("query-group-modal__datepicker", {
                ...
                "query-group-modal__datepicker--invalid":
                  !minDate && minTouched && hasActiveDate
              })}
              locale="de"
          ...
          <div className="query-group-modal__input-group">
            ...
            <DatePicker
              ...
              className={classnames("query-group-modal__datepicker", {
                ...
                "query-group-modal__datepicker--invalid":
                  !maxDate && maxTouched && hasActiveDate
              })}
              locale="de"
              dateFormat={localizedDateFormat()}
              selected={maxDate}
...
const mapDispatchToProps = (dispatch: any) => ({
  ...
  onSetMinDate: (andIdx, date) =>
    dispatch(queryGroupModalSetMinDate(andIdx, date)),
  onSetMaxDate: (andIdx, date) =>
    dispatch(queryGroupModalSetMaxDate(andIdx, date)),
  onResetAllDates: andIdx => dispatch(queryGroupModalResetAllDates(andIdx)),
  onSetTouched: (andIdx, field, value?) =>
    dispatch(queryGroupModalSetTouched(andIdx, field))
});
...
const mergeProps = (stateProps, dispatchProps, ownProps) => ({
  ...
  ...stateProps,
  ...dispatchProps,
  onSetMinDate: date => dispatchProps.onSetMinDate(stateProps.andIdx, date),
  onSetMaxDate: date => dispatchProps.onSetMaxDate(stateProps.andIdx, date),
  onResetAllDates: () => dispatchProps.onResetAllDates(stateProps.andIdx),
  onSetTouched: field => dispatchProps.onSetTouched(stateProps.andIdx, field)
});
