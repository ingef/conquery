...
const InputRange = (props: PropsType) => {
  ...
  const minFormattedValue =
    ...
    Math.round(minValue) / factor ||
    null;
  const maxFormattedValue =
    (formattedValue && formattedValue.max) ||
    Math.round(maxValue) / factor ||
    null;
  const exactFormattedValue =
    (formattedValue && formattedValue.exact) ||
    Math.round(exactValue) / factor ||
    null;
    ...
    if (type === "exact")
      ...
      if (
        ...
        props.input.onChange(
          ...
          {
            min: props.input.formattedValue
              ? props.input.formattedValue.min
              : null,
            max: props.input.formattedValue
              ? props.input.formattedValue.max
              : null,
            [type]: nextFormattedValue
          }
        );
    else props.input.onChange(null, null);
  };
