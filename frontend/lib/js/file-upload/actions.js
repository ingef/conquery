...
import { readFileAsText, cleanFileContent } from "../common/helpers/fileHelper";
...
import type { DraggedFileType, GenericFileType } from "./types";
...
export const loadFilesStart = () => ({ type: DROP_FILES_START });
export const loadFilesSuccess = (res: any) =>
  defaultSuccess(DROP_FILES_SUCCESS, res);
export const loadFilesError = (err: any) => defaultError(DROP_FILES_ERROR, err);
  ...
  return (dispatch: Dispatch) => {
    ...
    return readFileAsText(file).then(
      r => {
        ...
        if (values.length)
          return dispatch([loadFilesSuccess(r), type.callback(type)]);
...
export const dropFilesDateRangeType = (
  item: DraggedFileType,
