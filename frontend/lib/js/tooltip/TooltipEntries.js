...
import { formatDate, numberToThreeDigitArray } from "../common/helpers";
    ...
    <div className="tooltip-entries__info">
      <p className="tooltip-entries__number">
        {numberToThreeDigitArray(matchingEntries).map((threeDigits, i) => (
          <span key={i} className="tooltip-entries__digits">
            {threeDigits}
          </span>
        ))}
      </p>
...
const TooltipEntries = (props: PropsType) => {
  ...
  return (
    <div
      className={classnames(props.className, "tooltip-entries", {
    ...
    >
      <div className="tooltip-entries__right-container">
        ...
        {props.dateRange && _renderConceptDateRangeTooltip(props.dateRange)}
      </div>
    </div>
  );
};
