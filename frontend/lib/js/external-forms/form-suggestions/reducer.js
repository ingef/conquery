...
const updateFormFilterProperty = (state: Object, action: Object, property: Object): Object => {
...
const loadFormFilterSuggestionsStart = (state: Object, action: Object): Object => {
  return updateFormFilterProperty(state, action, { isLoading: true });
};

const loadFormFilterSuggestionsSuccess = (state: Object, action: Object): Object => {
  const { andIdx, orIdx, filterIdx, tableIdx, fieldName } = action.payload;
  ...
  return updateFormFilterProperty(state, action, {
    ...
    options: action.payload.suggestions
      // Remove everything but label and value
      .map(option => ({
        label: option.label,
        value: option.value,
      }))
...
const loadFormFilterSuggestionsError = (state: Object, action: Object): Object => {
  return updateFormFilterProperty(state, action, { isLoading: false })
}

export const createFormSuggestionsReducer = (formType: string, fieldNames: string[]) => {
  const reducerHandlers = fieldNames.map(fieldName => {
    const actionTypes = createActionTypes(formType, fieldName);

    return {
      [actionTypes.LOAD_FILTER_SUGGESTIONS_START]:
        loadFormFilterSuggestionsStart,
      [actionTypes.LOAD_FILTER_SUGGESTIONS_SUCCESS]:
        loadFormFilterSuggestionsSuccess,
      [actionTypes.LOAD_FILTER_SUGGESTIONS_ERROR]:
