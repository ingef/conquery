...
function mapStateToProps(state, ownProps) {
  ...
  const form = state.externalForms[formStoreName][formStoreFieldName]
  const { andIdx, orIdx } = form;

  const node = findNode(state.externalForms.form, ownProps.formType, ownProps.name, andIdx, orIdx);
