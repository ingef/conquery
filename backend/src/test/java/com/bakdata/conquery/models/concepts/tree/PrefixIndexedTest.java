...
public class PrefixIndexedTest {
	...
	public static void init() throws IOException, JSONException, ConfigurationException {
		...
		getTestKeys().map(args -> (String) args.get()[0]).forEach(dict::add);

		dict.compress();

		ObjectNode node = Jackson.MAPPER
			.readerFor(ObjectNode.class)
			.readValue(In.resource(PrefixIndexedTest.class, CONCEPT_SOURCE).asStream());
	...
	public void basic(String key, CalculatedValue<Map<String, Object>> rowMap) throws JSONException {
		...
		if (oldResult == null) {
			...
			assertThat(oldResult.getId()).isEqualTo(idxResult.getId());
		}
	}
	...
	public void cached(String key, CalculatedValue<Map<String, Object>> rowMap) throws JSONException {
		...
		assertThat(reference.getId()).describedAs("%s hierarchical name", key).isEqualTo(cached2.getId()).isEqualTo(cached.getId());
	}
