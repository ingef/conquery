...
public class IntrospectionDelegatingRealmTest {
	...
	public void tokenIntrospectionSimpleUserNew() {
		...
		ConqueryAuthenticationInfo expected = new ConqueryAuthenticationInfo(USER_1, USER1_TOKEN_WRAPPED, REALM, true);
		assertThat(info)
				.isEqualTo(expected);

		assertThat(info.getPrincipals()).isEqualTo(expected.getPrincipals());
