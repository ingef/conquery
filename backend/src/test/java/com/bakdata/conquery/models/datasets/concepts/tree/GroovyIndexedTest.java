...
public class GroovyIndexedTest {
	...
	public static void init() throws IOException, JSONException, ConfigurationException {
		...
		ObjectNode node = mapper.readerFor(ObjectNode.class)
			.readValue(
				GroovyIndexedTest.class.getResourceAsStream(CONCEPT_SOURCE));
		...
		final ObjectReader conceptReader = ((Injectable) values -> values.add(Validator.class, validator)).injectInto(
			mapper).readerFor(Concept.class);
