...
class TypeIdDaysInRangeMergerTest {
	...
	public void withGapMergeable() {
		...
		TypeIdDaysInRangeMerger merger = TypeIdDaysInRangeMerger
			.builder()
			.consumer(output::add)
		...
		List<CDateRange> fullMonths = getMonthsOf(2018).stream().filter(month -> month.getMax().getMonth().getValue() % 2 == 0).collect(
			Collectors.toList());

		List<TypeIdDaysInRangeMerger.DaysInRangeEntry> entries = fullMonths
			.stream()
			.map(month -> new DaysInRange(month, month.getDurationInDays()))
			.map(
				month -> new TypeIdDaysInRangeMerger.DaysInRangeEntry(
					0,
					new PPColumn[0],
					month,
					new Identifier(Lists.newArrayList(1)),
					"a"))
