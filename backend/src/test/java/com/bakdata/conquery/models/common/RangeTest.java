...
class RangeTest {
	...
	public void exactly() {
		...
		assertThat((IntPredicate) exactly::contains).accepts(5).rejects(6, 7, 4);

		assertThat((Predicate<Range<Integer>>) exactly::contains).accepts(Range.exactly(5), exactly)
			.rejects(
				Range.exactly(6),
				Range.exactly(7),
				Range.exactly(4));
	...
	public void atMost() {
		...
		assertThat((IntPredicate) range::contains).accepts(5, 4, CDateRange.NEGATIVE_INFINITY)
			.rejects(
				6,
				CDateRange.POSITIVE_INFINITY);

		assertThat((Predicate<Range<Integer>>) range::contains).accepts(Range.exactly(5), Range.atMost(4))
			.rejects(
				Range.exactly(6),
				Range.atLeast(5),
				Range.atLeast(4));
	...
	public void atLeast() {
		...
		assertThat((IntPredicate) range::contains).accepts(5, 6, CDateRange.POSITIVE_INFINITY)
			.rejects(
				4,
				CDateRange.NEGATIVE_INFINITY);

		assertThat((Predicate<Range<Integer>>) range::contains).rejects(
			Range.atLeast(4),
			Range.atMost(4),
			Range.exactly(4)).accepts(Range.exactly(5), Range.exactly(6), Range.atLeast(5), Range.atLeast(6));
	}
	...
	public void all() {
		...
		assertThat((IntPredicate) range::contains).accepts(
			5,
			CDateRange.POSITIVE_INFINITY,
			CDateRange.NEGATIVE_INFINITY);

		assertThat((Predicate<Range<Integer>>) range::contains).accepts(Range.exactly(5), Range.of(5, 10))
			.rejects(
				Range.atMost(5),
				Range.atLeast(6));
	}
	...
	public void contains() {
		...
		assertThat((IntPredicate) range::contains).accepts(5, 6, 7, 8, 9, 10)
			.rejects(
				CDateRange.NEGATIVE_INFINITY,
				4,
				11,
		...
		assertThat((Predicate<Range<Integer>>) range::contains).accepts(
			range,
			Range.of(5, 10),
			Range.exactly(7),
			Range.exactly(5),
	...
	@Test
	public void span() {
		assertThat(Range.exactly(5).span(Range.exactly(6))).isEqualTo(Range.of(5, 6))
			.isEqualTo(
				Range.exactly(6).span(Range.exactly(5)));

		assertThat(Range.of(5, 7).span(Range.exactly(6))).isEqualTo(Range.of(5, 7));
	}
	...
	public void coveredQuartersNotAFullQuarter() {
		...
		assertThat(dateRange.getCoveredQuarters()).containsExactlyInAnyOrder(
			CDateRange.of(LocalDate.of(2000, 1, 10), LocalDate.of(2000, 3, 15)));
	}
