...
public interface TestFunctionProvider {
	...
	default String createTableStatement(Table<Record> table, List<Field<?>> columns, DSLContext dslContext) {
		return dslContext.createTable(table).columns(columns).getSQL(ParamType.INLINED);
	}

	default void insertValuesIntoTable(
		Table<Record> table,
		List<Field<?>> columns,
		List<RowN> content,
		Statement statement,
	...
	default String createDropTableStatement(Table<Record> table, DSLContext dslContext) {
		return dslContext.dropTableIfExists(table).getSQL(ParamType.INLINED);
	}
