...
public class ScriptEndTest extends IntegrationTest.Simple implements ProgrammaticIntegrationTest {
	...
	public void execute(StandaloneSupport conquery) throws Exception {
		final URI scriptUri = HierarchyHelper.hierarchicalPath(
			conquery.defaultAdminURIBuilder(),
			AdminResource.class,
			"executeScript").build();

		try (Response resp = conquery.getClient()
			.target(scriptUri)
			.request(MediaType.TEXT_PLAIN_TYPE)
			.post(
				Entity.entity("storage", MediaType.TEXT_PLAIN_TYPE))) {
			assertThat(resp.getStatusInfo().getFamily()).isEqualTo(Response.Status.Family.SUCCESSFUL);

			assertThat(resp.readEntity(String.class)).contains(MetaStorage.class.getSimpleName());
		}
	}
}
