...
public class EntityExportTest implements ProgrammaticIntegrationTest {
	...
	public void execute(String name, TestConquery testConquery) throws Exception {
		...
		final SelectId valuesSelectId = SelectId.Parser.INSTANCE.parsePrefixed(
			dataset.getName(),
			"tree2.connector.values");
		{
			...
			final URI setPreviewConfig = HierarchyHelper.hierarchicalPath(
				conquery.defaultAdminURIBuilder(),
				AdminDatasetResource.class,
				"setPreviewConfig").buildFromMap(Map.of(ResourceConstants.DATASET, dataset));

			final PreviewConfig previewConfig = new PreviewConfig();

			previewConfig.setInfoCardSelects(
				List.of(
					new PreviewConfig.InfoCardSelect(
						"Age",
						SelectId.Parser.INSTANCE.parsePrefixed(dataset.getName(), "tree1.connector.age"),
						null),
					new PreviewConfig.InfoCardSelect("Values", valuesSelectId, null)
				));

			previewConfig.setTimeStratifiedSelects(
				List.of(
					new PreviewConfig.TimeStratifiedSelects(
						"Values in Time",
						"Description",
						List.of(
							new PreviewConfig.InfoCardSelect(
			...
			try (Response response = conquery.getClient()
				.target(setPreviewConfig)
				.request(
		...
		final URI entityExport = HierarchyHelper.hierarchicalPath(
			...
			"getEntityData").buildFromMap(Map.of(ResourceConstants.DATASET, conquery.getDataset().getName()));
		...
		assertThat(infos.columns()).containsExactly(
			new ColumnDescriptor(
				...
				"LIST[STRING]",
				Set.of(
					new SemanticType.SelectResultT(
						valuesSelectId
					))
			)
		);
		...
		assertThat(infos.years().get(0)).isEqualTo(
			new EntityPreviewStatus.YearEntry(
				2010,
				Map.of("Values", List.of("B2")),
				List.of(
		...
		assertThat(result.getColumnDescriptions()).isNotNull().isNotEmpty();

		final Optional<ColumnDescriptor> t2values = result.getColumnDescriptions()
			.stream()
			.filter(
				desc -> "table2 column".equals(desc.getLabel()))
			.findFirst();

		assertThat(t2values).isPresent();
		assertThat(t2values.get().getDescription()).isEqualTo("This is a column");
		assertThat(t2values.get().getSemantics()).contains(
			new SemanticType.ConceptColumnT(ConceptId.Parser.INSTANCE.parsePrefixed(dataset.getName(), "tree2"))
		);


		final Optional<URI> csvUrl = result.getResultUrls()
			.stream()
			.map(ResultAsset::url)
			.filter(
				url -> url.getPath().endsWith(".csv"))
			.findFirst();

		assertThat(csvUrl).isPresent();

		try (Response resultLines = conquery.getClient()
			.target(csvUrl.get())
			.queryParam("pretty", false)
			.request(
				AdditionalMediaTypes.CSV)
			.header("Accept-Language", "en-Us")
			.get()) {

			assertThat(resultLines.getStatusInfo().getFamily()).describedAs(
				new LazyTextDescription(() -> resultLines.readEntity(String.class)))
				.isEqualTo(
					Response.Status.Family.SUCCESSFUL);


			assertThat(
				resultLines.readEntity(String.class).lines().collect(Collectors.toList())).containsExactlyInAnyOrder(
