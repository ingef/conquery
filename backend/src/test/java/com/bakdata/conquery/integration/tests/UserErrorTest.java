	...
	public void execute(StandaloneSupport conquery) throws Exception {
		{
			...
			try (final Response response = request.post(Entity.entity(new CQAnd(), MediaType.APPLICATION_JSON_TYPE))) {
				assertThat(response.getStatus()).isEqualTo(HttpStatus.SC_BAD_REQUEST);
				assertThat(response.readEntity(HttpError.class)).isEqualTo(
					new HttpError(
						400,
...

		{
			...
			final UriBuilder getConceptNodeUri = HierarchyHelper.hierarchicalPath(
			...
			final Invocation.Builder request = conquery.getClient()
				...
				.resolveTemplates(
					Map.of(
						...
						conquery.getDataset().toString(),
						ResourceConstants.CONCEPT,
						unknownConcept.toString()
					)
				)
				.request(MediaType.APPLICATION_JSON_TYPE)
				.acceptLanguage(Locale.ENGLISH);
			try (final Response response = request.get()) {
				assertThat(response.getStatus()).describedAs(() -> response.readEntity(String.class))
					.isEqualTo(
						HttpStatus.SC_NOT_FOUND);
				assertThat(response.readEntity(HttpError.class)).isEqualTo(
					new HttpError(404, "The id UserErrorTest.unknown_concept could not be resolved.", null));
			}
		}
