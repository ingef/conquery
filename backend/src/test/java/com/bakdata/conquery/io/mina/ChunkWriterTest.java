...
public class ChunkWriterTest {
	...
	public void test(int messageLength) throws Exception {
		...
		List<IoBuffer> buffers = out.getMessageQueue().stream().map(IoBuffer.class::cast).collect(Collectors.toList());
		...
		assertThat(buffers.get(buffers.size() - 1))
			.satisfies(b -> assertThat(b.get(0)).as("last message is marked").isEqualTo(ChunkWriter.LAST_MESSAGE));
	}
}
