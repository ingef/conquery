...
public class FormConfigPatch extends MetaDataPatch {
	...
	public void applyTo(FormConfig instance, MetaStorage storage, Subject subject) {
		chain(QueryUtils.getNoOpEntryPoint(), storage, subject, instance).accept(this);
	}

	protected Consumer<FormConfigPatch> chain(
		Consumer<FormConfigPatch> patchConsumerChain,
		MetaStorage storage,
		Subject subject,
		FormConfig instance) {
