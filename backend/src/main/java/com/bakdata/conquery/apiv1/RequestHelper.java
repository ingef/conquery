...
public class RequestHelper {
	...
	public static String getHeader(ContainerRequestContext requestContext, String key, String defaultValue) {
		String header = requestContext.getHeaders().getFirst(key);
		return Optional.ofNullable(header).orElse(defaultValue);
	}

	public static String getUserAgent(ContainerRequestContext requestContext, String defaultValue) {
	...
	public static String getIp(HttpServletRequest request, String defaultValue) {
		return Optional
			.ofNullable(request)
			.map(r -> Optional.ofNullable(r.getHeader(AdditionalHeaders.HTTP_HEADER_REAL_IP)).orElse(r.getRemoteAddr()))
			.orElse(defaultValue);
	}
	...
	public static Optional<Principal> getPrincipal(ContainerRequestContext requestContext) {
		return getSecurityContext(requestContext).map(SecurityContext::getUserPrincipal);
	}
