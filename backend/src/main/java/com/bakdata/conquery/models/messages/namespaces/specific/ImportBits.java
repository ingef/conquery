...
public class ImportBits extends WorkerMessage.Slow {
	...
	public void react(Worker context) throws Exception {
		...
		for (Bit bit : bits) {
			try (ByteArrayInputStream input = new ByteArrayInputStream(bytes, bytePos, bit.getSize())) {
				 ...
				 * 
				 * for(int e=0;e<bit.getNumberOfEntries();e++) { input.readBytes(nullBytes);
				 * 
				 * Object[] row = new Object[t.getColumns().length]; for(int
				 * i=0;i<row.length;i++) { if(nulls.getBit(i)) row[i] =
				 * imp.getColumns()[i].getType().kryoRead(kryo, input); } events.add(row); }
				 */
				newBlocks.add(factory.readBlock(bit.getId(), imp, input));
				if (input.available() > 0) {
					throw new IllegalStateException(
						"After reading the bit " + bit + " there are still " + input.available() + " bytes remaining in its content");
				}
	...
	public String toString() {
		return "Importing " + bits.size() + " from " + FileUtils.byteCountToDisplaySize(bytes.length) + " as " + tag + " into " + table;
	}
