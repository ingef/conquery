...
public class DurationSumFilter extends SingleColumnFilter<Range.LongRange> {
	...
	public void configureFrontend(FEFilter f) throws ConceptConfigurationException {
		switch (getColumn().getType()) {
			// TODO DATE is not supported according to getAcceptedColumnTypes
			case DATE, DATE_RANGE -> {
				f.setType(FEFilterType.INTEGER_RANGE);
				f.setMin(0);
			}

			default -> throw new ConceptConfigurationException(getConnector(), "DURATION_SUM filter is incompatible with columns of type "
																			   + getColumn().getType());
		}
	}
