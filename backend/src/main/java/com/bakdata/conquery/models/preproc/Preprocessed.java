...
public class Preprocessed {
	...
	private void writeRowToFile(Import imp, int entityId, List<Object[]> events) throws IOException {
		...
		// we can't map the primary column since we do a lot of work which would destroy
		// any compression anyway
		// entityId =
		// (Integer)primaryColumn.getType().transformFromMajorType(primaryColumn.getOriginalType(),
		// Integer.valueOf(entityId));

		for (ImportColumn ic : imp.getColumns()) {
			for (Object[] event : events) {
				if (event[ic.getPosition()] != null) {
					event[ic.getPosition()] = ic
						.getType()
						.transformFromMajorType(columns[ic.getPosition()].getOriginalType(), event[ic.getPosition()]);
				}
			}
		}
