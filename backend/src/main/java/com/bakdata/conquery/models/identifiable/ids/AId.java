...
public abstract class AId<TYPE> {
	...
	public final static char JOIN_CHAR = '.';
	public final static Joiner JOINER = Joiner.on(JOIN_CHAR);
	public final static Map<Class<?>, Class<?>> CLASS_TO_ID_MAP = new ConcurrentHashMap<>();
	...
	static void checkConflict(AId<?> id, AId<?> cached) {
		if (!cached.equals(id)) {
	...
	public static <T extends AId<?>> Class<T> findIdClass(Class<?> cl) {
		...
		try {
			...
			if (!AId.class.isAssignableFrom(returnType) || AId.class.equals(returnType)) {
				throw new IllegalStateException("The type `" + returnType + "` of `" + cl + "#" + methodName + "` is not a specific subtype of AId");
			}
	...
	public String toString() {
		...
		return AId.JOINER.join(components);
	}
	...
	public interface Parser<ID extends AId> {
		...
		static String[] split(String id) {
			...
			String[] parts = StringUtils.split(id, AId.JOIN_CHAR);
			...
			if (remaining.remaining() > 0) {
				throw new IllegalStateException(
						String.format(
								...
								AId.JOINER.join(allParts),
								id,
								id.getClass().getSimpleName(),
								AId.JOINER.join(remaining.getRemaining())
						)
				);
			}
			return id;
