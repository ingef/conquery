...
public class Workers extends IdResolveContext {
	...
	@JsonIgnore
	private final transient ConcurrentHashMap<DatasetId, Worker> workers = new ConcurrentHashMap<>();
	...
	public Worker createWorker(Dataset dataset, StoreFactory storageConfig, @NonNull String dirName, Validator validator, boolean failOnError) {
		...
		final Worker
				worker =
				Worker.newWorker(dataset, queryThreadPoolDefinition, jobsThreadPool, storageConfig, dirName, validator, failOnError, entityBucketSize, persistenceMapper, communicationMapper, secondaryIdSubPlanRetention);
