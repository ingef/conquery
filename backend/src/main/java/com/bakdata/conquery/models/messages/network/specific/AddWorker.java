...
public class AddWorker extends MessageToShardNode.Slow {
	...
	public void react(ShardNodeNetworkContext context) throws Exception {
		...
		Worker worker = context.getWorkers().createWorker(dataset, config.getStorage(), createWorkerName(), context.getValidator(), config.isFailOnError(), context.getShardNode().getEnvironment().metrics());
