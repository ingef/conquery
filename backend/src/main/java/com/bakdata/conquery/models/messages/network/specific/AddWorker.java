...
public class AddWorker extends MessageToShardNode.Slow {
	...
	public void react(ShardNodeNetworkContext context) throws Exception {
		...
		Worker worker = context.getWorkers().createWorker(dataset, config.getStorage(), workerDirectory(dataset), context.getValidator(), config.isFailOnError());
	...
	private static String workerDirectory(Dataset dataset) {
