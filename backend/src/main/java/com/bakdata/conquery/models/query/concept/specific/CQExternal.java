...
public class CQExternal implements CQElement {
	...
	public static enum DateFormat {
		EVENT_DATE {
			...
			public CDateSet readDates(int[] dateIndices, String[] row) throws ParsingException {
				return CDateSet.create(Collections.singleton(
					CDateRange.exactly(DateFormats.instance().parseToLocalDate(row[dateIndices[0]]))
				));
			}
		},
		START_END_DATE {
			...
			public CDateSet readDates(int[] dateIndices, String[] row) throws ParsingException {
				...
				LocalDate end = (dateIndices.length<2 || row[dateIndices[1]]==null) ? null : DateFormats.instance().parseToLocalDate(row[dateIndices[1]]);
		...
		DATE_RANGE {
			...
			public CDateSet readDates(int[] dateIndices, String[] row) throws ParsingException {
				return CDateSet.create(Collections.singleton(
					DateRangeType.parseISORange(row[dateIndices[0]])
				));
			}
		},
		DATE_SET {
	...
	public CQElement resolve(QueryResolveContext context) {
		...
		for(String[] row : values) {
			...
			int entity = primary.getId(row[idIndex]);
			
			//read the dates from the row
			try {
				CDateSet dates = dateFormat
					.map(df -> {
						try {
				...
				includedEntities.put(entity, Objects.requireNonNull(dates));
			} catch(Exception e) {
