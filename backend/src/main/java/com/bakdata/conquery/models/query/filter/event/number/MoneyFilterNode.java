...
public class MoneyFilterNode extends NumberFilterNode<Range.LongRange> {
	...
	public MoneyFilterNode(SingleColumnFilter filter, FilterValue<Range.LongRange> filterValue) {
	...
	public boolean contains(Block block, int event) {
		return getRange().contains(block.getMoney(event, filter.getColumn()));
	}
}
