...
public class RelExportGenerator {
	...
	public static RelativeFormQuery generate(RelativeMode mode) {
		...
		List<ExportForm.ResolutionAndAlignment> resolutionsAndAlignments = ExportForm.getResolutionAlignmentMap(
			mode.getForm().getResolvedResolutions(),
			mode.getTimeUnit().getAlignment());

		return generate(
			mode.getForm().getPrerequisite(),
			mode.getResolvedFeatures(),
	...
	public static RelativeFormQuery generate(
		Query query,
		ArrayConceptQuery features,
		TemporalSamplerFactory indexSelector,
