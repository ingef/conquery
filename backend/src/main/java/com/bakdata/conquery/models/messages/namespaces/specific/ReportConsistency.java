...
public class ReportConsistency extends NamespaceMessage {
	...
	private static <ID extends NamespacedId<?>> boolean isConsistent(
		String typeName,
		@NonNull Set<ID> managerIds,
		@NonNull Set<ID> workerIds,
		WorkerId workerId) {
		...
		if (notInWorker.isEmpty() && notInManager.isEmpty()) {
			log.info(
				"{} of worker {} are consistent with the manager: {} {}",
				typeName,
				workerId,
				managerIds.size(),
				typeName);
		...
		for (ID difference : notInWorker) {
			sb.append("\t[")
				.append(difference)
				.append("] is not present on the worker but on the manager [")
				.append(
					workerId)
				.append("].\n");
		}
		for (ID difference : notInManager) {
			sb.append("\t[")
				.append(difference)
				.append("] is not present on the manager but on the worker [")
				.append(
					workerId)
				.append("].\n");
		}
