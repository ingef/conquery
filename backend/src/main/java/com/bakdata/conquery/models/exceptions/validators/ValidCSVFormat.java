		...
		public boolean isValid(List<FormatColumn> value, ConstraintValidatorContext context) {
			...
			if (value != null) {
				...
				if (value.stream().anyMatch(Objects::isNull)) {
					context.buildConstraintViolationWithTemplate("Null is not allowed as part of the format").addConstraintViolation();
					correct = false;
				}
				...
				if (!value.contains(FormatColumn.ID)) {
					context.buildConstraintViolationWithTemplate("The format does not contain " + FormatColumn.ID).addConstraintViolation();
					correct = false;
				}
				long dateTypes = value.stream().map(FormatColumn::getDateFormat).filter(Objects::nonNull).distinct().count();
				if (dateTypes > 1) {
					context
						.buildConstraintViolationWithTemplate(
							"The format may not contain more than one of DATE_RANGE, EVENT_DATE and (START_DATE and/or END_DATE)")
						.addConstraintViolation();
					correct = false;
				}
