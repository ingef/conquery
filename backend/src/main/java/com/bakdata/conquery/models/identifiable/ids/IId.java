...
public interface IId<TYPE> {
	...
	interface Parser<ID extends IId<?>> {
		...
		default ID parsePrefixed(String dataset, String id) {
			...
			List<String> parts = ImmutableList.<String>builderWithExpectedSize(split.length + 1).add(dataset).add(split).build();
			return parse(parts);
		}
			...
			try {
				...
				try {
					if (AId.class.isAssignableFrom(returnType) && !AId.class.equals(returnType)) {
						...
						throw new IllegalStateException(
							"The type " + returnType + " of " + methodName + " is not a specific subtype of IId");
					}
				}
