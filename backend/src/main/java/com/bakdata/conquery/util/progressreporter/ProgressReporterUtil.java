	...
	/* package */ String buildProgressReportString(boolean done, double progress, long elapsed, long waited) {
		if (progress == 0) {
			return new StringBuilder()
				.append("waited ")
				.append(ProgressReporterUtil.TIME_FORMATTER.format(LocalTime.MIDNIGHT.plus(Duration.ofNanos(waited))))
			...
			return new StringBuilder()
				...
				.append(ProgressReporterUtil.TIME_FORMATTER.format(LocalTime.MIDNIGHT.plus(estimate)))
				.toString();
		}
	}
}
