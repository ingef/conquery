...
public class SmallOut extends OutputStream implements AutoCloseable {
	 ...
	 *            this are written and {@link #flush()} does not empty the buffer.
	 */
	public SmallOut(int bufferSize) {
	 ...
	 * @param maxBufferSize
	 *            If {@link #flush()} does not empty the buffer, the buffer is
	 *            doubled as needed until it exceeds maxBufferSize and an exception
	 *            is thrown. Can be -1 for no maximum.
	 */
	public SmallOut(int bufferSize, int maxBufferSize) {
		...
		this.maxCapacity = maxBufferSize == -1 ? MAX_ARRAY_SIZE : maxBufferSize;
	 ...
	 */
	public SmallOut(byte[] buffer, int maxBufferSize) {
		if (buffer == null)
	...
	/**
	 * Creates a new Output for writing to an OutputStream. A buffer size of 4096 is
	 * used.
	 */
	public SmallOut(OutputStream outputStream) {
		this(4096, 4096);
	...
	/**
	 * Creates a new Output for writing to an OutputStream with the specified buffer
	 * size.
	 */
	public SmallOut(OutputStream outputStream, int bufferSize) {
		this(bufferSize, bufferSize);
		if (outputStream == null)
			throw new IllegalArgumentException("outputStream cannot be null.");
		this.outputStream = outputStream;
	...
	/**
	 * Sets a new OutputStream to flush data to when the buffer is full. The
	 * position and total are reset, discarding any buffered bytes.
	 * 
	 * @param outputStream
	 *            May be null.
	 */
	...
	/**
	 * Sets a new buffer to write to. The bytes are not copied, the old buffer is
	 * discarded and the new buffer used in its place. The position and total are
	 * reset. The {@link #setOutputStream(OutputStream) OutputStream} is set to
	 * null.
	 * 
	 * @param maxBufferSize
	 *            If {@link #flush()} does not empty the buffer, the buffer is
	...
	/**
	 * Allocates and returns a new byte[] containing the bytes currently in the
	 * buffer between 0 and {@link #position()}.
	 */
	public byte[] toBytes() {
	...
	/**
	 * If false, {@link #writeInt(int, boolean)}, {@link #writeLong(long, boolean)},
	 * {@link #writeInts(int[], int, int, boolean)}, and
	 * {@link #writeLongs(long[], int, int, boolean)} will use fixed length
	 * encoding, which may be faster for some data. Default is true.
	 */
	...
	/**
	 * Returns the current position in the buffer. This is the number of bytes that
	...
	/**
	 * Returns the total number of bytes written. This may include bytes that have
	 * not been flushed.
	 */
	public long total() {
	...
	protected boolean require(int required) throws IOException {
		...
		if (required > maxCapacity - position) {
			...
			throw new IOException("Buffer overflow. Available: " + (maxCapacity - position) + ", required: " + required);
		}
		if (capacity == 0)
			capacity = 16;
		do {
	...
	/**
	 * Flushes the buffered bytes. The default implementation writes the buffered
	 * bytes to the {@link #getOutputStream() OutputStream}, if any, and sets the
	 * position to 0. Can be overridden to flush the bytes somewhere else.
	 */
	@Override
	 ...
	 * {@link #writeVarInt(int, boolean)} explicitly when writing values that should
	 ...
	 */
	public int writeVarInt(int value, boolean optimizePositive) throws IOException {
		if (!optimizePositive)
	 ...
	 * @return The number of bytes written.
	 */
	public int writeVarIntFlag(boolean flag, int value, boolean optimizePositive) throws IOException {
		if (!optimizePositive)
			value = (value << 1) ^ (value >> 31);
	 ...
	 * {@link #setVariableLengthEncoding(boolean)}. Use
	 * {@link #writeVarLong(long, boolean)} explicitly when writing values that
	 * should always use variable length encoding (eg values that appear many
	 * times).
	 * 
	 * @return The number of bytes written.
	 ...
	 */
	public int writeVarLong(long value, boolean optimizePositive) throws IOException {
		if (!optimizePositive)
	 ...
	 * @return The number of bytes written.
	 */
	public int writeVarFloat(float value, float precision, boolean optimizePositive) throws IOException {
	 ...
	 * @return The number of bytes written.
	 */
	public int writeVarDouble(double value, double precision, boolean optimizePositive) throws IOException {
	...
	private void writeUtf8_slow(String value, int charCount, int charIndex) throws IOException {
	...
	private void writeAscii_slow(String value, int charCount) throws IOException {
		if (charCount == 0)
			return;
		if (position == capacity)
			require(1); // Must be able to write at least one character.
		int charIndex = 0;
		byte[] buffer = this.buffer;
	...
	/**
	 * Writes an int array in bulk. This may be more efficient than writing them
	 * individually.
	 */
	public void writeInts(int[] array, int offset, int count) throws IOException {
		if (capacity >= count << 2) {
			require(count << 2);
			byte[] buffer = this.buffer;
			int p = position;
	...
	/**
	 * Writes an int array in bulk using fixed or variable length encoding,
	 * depending on {@link #setVariableLengthEncoding(boolean)}. This may be more
	 * efficient than writing them individually.
	 */
	public void writeInts(int[] array, int offset, int count, boolean optimizePositive) throws IOException {
		if (varEncoding) {
			for (int n = offset + count; offset < n; offset++)
	...
	/**
	 * Writes a long array in bulk. This may be more efficient than writing them
	 * individually.
	 */
	public void writeLongs(long[] array, int offset, int count) throws IOException {
		if (capacity >= count << 3) {
			require(count << 3);
			byte[] buffer = this.buffer;
			int p = position;
	...
	/**
	 * Writes a long array in bulk using fixed or variable length encoding,
	 * depending on {@link #setVariableLengthEncoding(boolean)}. This may be more
	 * efficient than writing them individually.
	 */
	public void writeLongs(long[] array, int offset, int count, boolean optimizePositive) throws IOException {
		if (varEncoding) {
			for (int n = offset + count; offset < n; offset++)
	...
	/**
	 * Writes a float array in bulk. This may be more efficient than writing them
	 * individually.
	 */
	public void writeFloats(float[] array, int offset, int count) throws IOException {
		if (capacity >= count << 2) {
			require(count << 2);
			byte[] buffer = this.buffer;
			int p = position;
	...
	/**
	 * Writes a double array in bulk. This may be more efficient than writing them
	 * individually.
	 */
	public void writeDoubles(double[] array, int offset, int count) throws IOException {
		if (capacity >= count << 3) {
			require(count << 3);
			byte[] buffer = this.buffer;
			int p = position;
	...
	/**
	 * Writes a short array in bulk. This may be more efficient than writing them
	 * individually.
	 */
	public void writeShorts(short[] array, int offset, int count) throws IOException {
		if (capacity >= count << 1) {
			require(count << 1);
			byte[] buffer = this.buffer;
			int p = position;
	...
	/**
	 * Writes a char array in bulk. This may be more efficient than writing them
	 * individually.
	 */
	public void writeChars(char[] array, int offset, int count) throws IOException {
		if (capacity >= count << 1) {
			require(count << 1);
			byte[] buffer = this.buffer;
			int p = position;
	...
	/**
	 * Writes a boolean array in bulk. This may be more efficient than writing them
	 * individually.
	 */
	public void writeBooleans(boolean[] array, int offset, int count) throws IOException {
		if (capacity >= count) {
			require(count);
			byte[] buffer = this.buffer;
			int p = position;
	...
	/**
	 * Returns the number of bytes that would be written with
	 * {@link #writeVarLong(long, boolean)}.
	 */
