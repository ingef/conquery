...
public class FilterValueSearch {
	...
	private @NotNull AutoCompleteResult sendQuery(
		String queryString,
		Integer start,
		@CheckForNull Integer limit,
		boolean withEmptySource,
		boolean sort) {
		...
		try {
			...
			log.debug(
				"Query [{}] Found: {} | Collected: {} | QTime: {} | ElapsedTime: {}",
				queryHash,
				numFound.get(),
				beans.size(),
				response.getQTime(),
	 ...
	 */
	private @NotNull SolrQuery buildSolrQuery(
		String queryString,
		Integer start,
		@CheckForNull Integer limit,
		boolean sort,
		boolean withEmptySource,
		boolean collapseSources) {
		...
		if (collapseSources) {
			...
			query.addFilterQuery(
				"{!collapse field=%s min=%s nullPolicy=expand}".formatted(
					SolrFrontendValue.Fields.value_s,
					SolrFrontendValue.Fields.sourcePriority_i));
		}
	...
	public ConceptsProcessor.ExactFilterValueResult exact(Collection<String> terms) {
		...
		final Map<String, String> unresolvedMap = terms.stream()
			.collect(
				Collectors.toMap(String::toLowerCase, Function.identity(), (v1, v2) -> v1));
			...
			try {
				...
				QueryResponse response = solrClient.queryAndStreamResponse(solrQuery, new StreamingResponseCallback() {
					...
					public void streamSolrDocument(SolrDocument doc) {
						...
						if (unresolvedMap.remove(
							bean.value_s.toLowerCase()) != null || (bean.label_t != null && unresolvedMap.remove(
								bean.label_t.toLowerCase()) != null)) {
					...
					public void streamDocListInfo(long numFoundCallBack, long start, Float maxScore) {
						...
						if (numFoundCallBack > batchSize) {
							log.warn(
								"Query found more documents ({}) than expected ({}). We expect a term to be found in at most {} documents (from a column and a mapping).",
								numFoundCallBack,
								batchSize,
								source_count);
						}
