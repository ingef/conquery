...
public class SmallIn extends InputStream implements AutoCloseable {
	...
	/**
	 * Creates an uninitialized Input, {@link #setBuffer(byte[])} must be called
	 ...
	 * 
	 * @param bufferSize
	 *            The size of the buffer. An exception is thrown if more bytes than
	 *            this are read and {@link #fill(byte[], int, int)} does not supply
	 *            more bytes.
	 */
	public SmallIn(int bufferSize) {
	 ...
	 * 
	 * @param buffer
	 *            An exception is thrown if more bytes than this are read and
	 *            {@link #fill(byte[], int, int)} does not supply more bytes.
	 */
	public SmallIn(byte[] buffer) {
	 ...
	 *            {@link #fill(byte[], int, int)} does not supply more bytes.
	 */
	public SmallIn(byte[] buffer, int offset, int count) {
	 ...
	 * 4096.
	 */
	public SmallIn(InputStream inputStream) {
		this(4096);
		if (inputStream == null)
	...
	/**
	 * Creates a new Input for reading from an InputStream with the specified buffer
	 * size.
	 */
	public SmallIn(InputStream inputStream, int bufferSize) {
		this(bufferSize);
		if (inputStream == null)
			throw new IllegalArgumentException("inputStream cannot be null.");
		this.inputStream = inputStream;
	...
	/**
	 * Sets a new buffer to read from. The bytes are not copied, the old buffer is
	 * discarded and the new buffer used in its place. The position and total are
	 * reset. The {@link #setInputStream(InputStream) InputStream} is set to null.
	 */
	...
	/**
	 * Returns the buffer. The bytes between 0 and {@link #position()} are the data
	 * that can be read.
	 */
	...
	/**
	 * Sets an InputStream to read from when data in the buffer is exhausted. The
	 * position, limit, and total are reset, discarding any buffered bytes.
	 * 
	 * @param inputStream
	 *            May be null.
	 */
	...
	/**
	 * If false, {@link #readInt(boolean)}, {@link #readLong(boolean)},
	 * {@link #readInts(int, boolean)}, and {@link #readLongs(int, boolean)} will
	 * use fixed length encoding, which may be faster for some data. Default is
	 * true.
	 */
	...
	/**
	 * Sets the limit in the buffer which marks the end of the data that can be
	 * read.
	 */
	...
	/**
	 * Fills the buffer with more bytes. The default implementation reads from the
	 * {@link #getInputStream() InputStream}, if set. Can be overridden to fill the
	 * bytes from another source.
	 * 
	 * @return -1 if there are no more bytes.
	 */
	protected int fill(byte[] buffer, int offset, int count) throws IOException {
		if (inputStream == null)
			return -1;
	 ...
	 * @param required
	 *            Must be > 0.
	 * @return The number of bytes remaining in the buffer, which will be at least
	 *         <code>required</code> bytes.
	 * @throws IOException
	 *             if {@link #fill(byte[], int, int)} is unable to provide more
	 *             bytes (buffer underflow).
	 */
	protected int require(int required) throws IOException {
	 ...
	 */
	protected int optional(int optional) throws IOException {
		int remaining = limit - position;
	...
	public long skip(long count) throws IOException {
		...
		while (remaining > 0) {
			int skip = (int) Math.min(SmallOut.MAX_ARRAY_SIZE, remaining);
			skip(skip);
			remaining -= skip;
		}
		return count;
	 ...
	 * offset.
	 */
	public void readBytes(byte[] bytes, int offset, int count) throws IOException {
		if (bytes == null)
			throw new IllegalArgumentException("bytes cannot be null.");
		int copyCount = Math.min(limit - position, count);
		while (true) {
	 ...
	 * {@link #setVariableLengthEncoding(boolean)}. Use {@link #readVarInt(boolean)}
	 * explicitly when reading values that should always use variable length
	 * encoding (eg values that appear many times).
	 * 
	 * @see #canReadInt()
	 */
	...
	private int readVarInt_slow(boolean optimizePositive) throws IOException {
	...
	/**
	 * Returns true if enough bytes are available to read an int with
	 * {@link #readVarInt(boolean)}.
	 */
	public boolean canReadVarInt() throws IOException {
		if (limit - position >= 5)
			return true;
		if (optional(5) <= 0)
			return false;
	...
	/**
	 * Reads the boolean part of a varint flag. The position is not advanced,
	 * {@link #readVarIntFlag(boolean)} should be used to advance the position.
	 * @throws IOException 
	 */
	public boolean readVarIntFlag() throws IOException {
		if (position == limit)
			require(1);
	...
	/**
	 * Reads the 1-5 byte int part of a varint flag. The position is advanced so if
	 * the boolean part is needed it should be read first with
	 * {@link #readVarIntFlag()}.
	 * @throws IOException 
	 */
	public int readVarIntFlag(boolean optimizePositive) throws IOException {
		if (require(1) < 5)
	...
	private int readVarIntFlag_slow(boolean optimizePositive) throws IOException {
		// The buffer is guaranteed to have at least 1 byte.
		int b = buffer[position++];
		int result = b & 0x3F;
		if ((b & 0x40) != 0) {
	 ...
	 * {@link #setVariableLengthEncoding(boolean)}. Use
	 * {@link #readVarLong(boolean)} explicitly when reading values that should
	 * always use variable length encoding (eg values that appear many times).
	 * 
	 * @see #canReadLong()
	 */
	public long readLong(boolean optimizePositive) throws IOException {
		if (varEncoding)
	...
	private long readVarLong_slow(boolean optimizePositive) throws IOException {
		// The buffer is guaranteed to have at least 1 byte.
		int b = buffer[position++];
		long result = b & 0x7F;
		if ((b & 0x80) != 0) {
	 ...
	 */
	public boolean canReadLong() throws IOException {
		if (varEncoding)
	...
	/**
	 * Returns true if enough bytes are available to read a long with
	 * {@link #readVarLong(boolean)}.
	 */
	public boolean canReadVarLong() throws IOException {
		if (limit - position >= 9)
			return true;
		if (optional(5) <= 0)
			return false;
	...
	public float readVarFloat(float precision, boolean optimizePositive) throws IOException {
	...
	public double readVarDouble(double precision, boolean optimizePositive) throws IOException {
	 ...
	 * strings written by {@link Output#writeString(String)} and
	...
	/**
	 * Reads the length and string of UTF8 characters, or null. For non-ASCII
	 * strings, this method avoids allocating a string by reading directly to the
	 * StringBuilder. This can read strings written by
	...
	public StringBuilder readStringBuilder() throws IOException {
		if (!readVarIntFlag())
			return new StringBuilder(readAsciiString()); // ASCII.
		// Null, empty, or UTF8.
		int charCount = readVarIntFlag(true);
		switch (charCount) {
			case 0:
	...
	private void readUtf8Chars(int charCount) throws IOException {
	...
	private void readUtf8Chars_slow(int charCount, int charIndex) throws IOException {
		char[] chars = this.chars;
		byte[] buffer = this.buffer;
	...
	private String readAscii_slow(int charCount) throws IOException {
		char[] chars = this.chars;
		byte[] buffer = this.buffer;
		while (true) {
			if (position == limit)
	...
	/**
	 * Reads an int array in bulk. This may be more efficient than reading them
	 * individually.
	 */
	...
	/**
	 * Reads an int array in bulk using fixed or variable length encoding, depending
	 * on {@link #setVariableLengthEncoding(boolean)}. This may be more efficient
	 * than reading them individually.
	 */
	public int[] readInts(int length, boolean optimizePositive) throws IOException {
		if (varEncoding) {
			int[] array = new int[length];
			for (int i = 0; i < length; i++)
	...
	/**
	 * Reads a long array in bulk. This may be more efficient than reading them
	 * individually.
	 */
	 ...
	 * than reading them individually.
	 */
	public long[] readLongs(int length, boolean optimizePositive) throws IOException {
		if (varEncoding) {
			long[] array = new long[length];
			for (int i = 0; i < length; i++)
	...
	/**
	 * Reads a float array in bulk. This may be more efficient than reading them
	 * individually.
	 */
	...
	/**
	 * Reads a double array in bulk. This may be more efficient than reading them
	 * individually.
	 */
	...
	/**
	 * Reads a short array in bulk. This may be more efficient than reading them
	 * individually.
	 */
	...
	/**
	 * Reads a char array in bulk. This may be more efficient than reading them
	 * individually.
	 */
	...
	/**
	 * Reads a boolean array in bulk. This may be more efficient than reading them
	 * individually.
	 */
	public boolean[] readBooleans(int length) throws IOException {
