...
public class BinaryJacksonCoder implements CQCoder<NetworkMessage<?>> {
	...
	public BinaryJacksonCoder(NamespacedStorageProvider namespacedStorageProvider, Validator validator, ObjectMapper objectMapper) {
		this.validator = validator;
		writer = objectMapper.writerFor(NetworkMessage.class);
		reader = namespacedStorageProvider.injectIntoNew(objectMapper.readerFor(NetworkMessage.class)).without(Feature.AUTO_CLOSE_SOURCE);
	}
