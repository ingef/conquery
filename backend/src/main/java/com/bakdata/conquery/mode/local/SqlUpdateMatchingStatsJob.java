...
public class SqlUpdateMatchingStatsJob extends Job {
	...
	private class MatchingStatsTask implements Callable<Void> {
		...
		public Void call() {
			...
			executionService.fetchStream(query).forEach(record -> mapRecordToConceptElements(record, relevantColumnsAliased, treeCache));
			return null;
		}

		/**
		 * @return A map from a connector to all relevant columns the connector's concept defines. A relevant column is any column that is used by a {@link CTCondition}
		 * which is part of any child of a concept, or it's a concept's connector column.
		 */
			...
			return treeConcept.getConnectors().stream().collect(Collectors.toMap(
					...
					connector -> {
						try {
							...
							log.error("Could not calculate matching stats for Concept {} because of a configuration error", treeConcept.getName());
		...
		private Set<String> collectRelevantColumns(final Connector connector, final List<ConceptTreeChild> children) throws ConfigurationException {
		...
		private void mapRecordToConceptElements(final Record record, final List<Field<?>> relevantColumns, final ConceptTreeCache treeCache) {
