...
public class ClusterNamespaceHandler implements NamespaceHandler<DistributedNamespace> {
	...
	public DistributedNamespace createNamespace(
		...
		NamespaceSetupData namespaceData = NamespaceHandler.createNamespaceSetup(
			...
			datasetRegistry,
			environment);
		DistributedExecutionManager executionManager = new DistributedExecutionManager(
			metaStorage,
			datasetRegistry,
			clusterState,
			config);
	...
	public void removeNamespace(DatasetId id, DistributedNamespace namespace) {
		clusterState.getShardNodes()
			.values()
			.forEach(
				shardNode -> shardNode.send(new RemoveWorker(namespace.getDataset().getId())));
