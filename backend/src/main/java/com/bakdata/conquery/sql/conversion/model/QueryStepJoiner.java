...
public class QueryStepJoiner {
	...
	public static QueryStep antiJoinWithAllIdsTable(
		...
		Table<?> table = table(name(allIdsTable)).leftOuterJoin(table(name(queryStep.getCteName())))
			.on(
				allIdsPrimaryColumn.eq(queryStepPrimaryColumn));
		...
		Selects selects = Selects.builder()
			...
			.validityDate(
				validityDate.map(cdr -> cdr.asValidityDateRange(cteName)))
			.build();
		...
		return QueryStep.builder()
			...
			.fromTable(table)
			.conditions(
				List.of(queryStepPrimaryColumn.isNull()))
	...
	private static QueryStep doJoin(
		...
		// if there is only 1 child node containing a validity date, we just keep it as overall validity date for the joined node
			...
			joinedStep = buildStepAndAggregateDates(
				ids,
				mergedSelects,
				joinedStepBuilder,
	...
	private static QueryStep joinStepsContainingNegation(
		...
		Map<Boolean, List<QueryStep>> byNegation = queriesToJoin.stream()
			.collect(
				Collectors.groupingBy(QueryStep::isNegate));
		...
		Selects.SelectsBuilder selects = nonNegateJoined.getQualifiedSelects()
			.toBuilder()
			.sqlSelects(
				mergeSelects(List.of(nonNegateJoined, negateJoined)));


		if (logicalOperation == ConqueryJoinType.INNER_JOIN) {
			Table<?> table = table(name(nonNegateJoined.getCteName())).leftOuterJoin(
				table(name(negateJoined.getCteName())))
				.on(
					nonNegateJoined.getQualifiedSelects()
						.getIds()
						.getPrimaryColumn()
						.eq(
							negateJoined.getQualifiedSelects().getIds().getPrimaryColumn()));
			...
			return QueryStep.builder()
				...
				.fromTable(table)
				.conditions(
					List.of(negateJoined.getQualifiedSelects().getIds().getPrimaryColumn().isNull()))
				.predecessors(
					List.of(nonNegateJoined, negateJoined))
				.build();
		}
		...
		DateAggregationDates aggregationDates = DateAggregationDates.forValidityDates(
			List.of(
		...
		ColumnDateRange merged = dateAggregator.getAggregatedValidityDate(
			aggregationDates,
			DateAggregationAction.MERGE).as(cteName + SharedAliases.DATES_COLUMN.getAlias());

		Field<String> coalescedId = DSL.coalesce(nonNegatePrimaryColumn, allIdsPrimaryColumn)
			.as(
				SharedAliases.PRIMARY_COLUMN.getAlias());

		selects = selects.ids(new SqlIdColumns(coalescedId)).validityDate(Optional.of(merged));

		Table<?> table = table(name(context.getIdColumns().getTable())).leftOuterJoin(
			table(name(negateJoined.getCteName())))
			.on(allIdsPrimaryColumn.eq(negatePrimaryColumn))
			.leftOuterJoin(
				table(name(nonNegateJoined.getCteName())))
		...
		return QueryStep.builder()
			...
			.fromTable(table)
			.conditions(
				List.of(negatePrimaryColumn.isNull().or(nonNegatePrimaryColumn.isNotNull())))
			.predecessors(
				List.of(nonNegateJoined, negateJoined))
			.build();
	...
	public static TableLike<Record> constructJoinedTable(
		List<QueryStep> queriesToJoin,
		ConqueryJoinType logicalOperation,
		ConversionContext context) {
		...
		Function3<Table<?>, Table<?>, List<Condition>, TableOnConditionStep<Record>> joinType = switch (logicalOperation) {
		...
		for (int i = 0; i < queriesToJoin.size() - 1; i++) {
			...
			List<Condition> joinConditions = Stream.concat(
				joinIdsCondition.stream(),
				Stream.of(joinDateCondition)).collect(Collectors.toList());
	...
	private static Condition joinOnStratification(QueryStep leftPartQS, QueryStep rightPartQS) {
		if (leftPartQS.getSelects().getStratificationDate().isEmpty() || rightPartQS.getSelects()
			.getStratificationDate()
			.isEmpty()) {
	...
	public static List<SqlSelect> mergeSelects(List<QueryStep> querySteps) {
		return querySteps.stream()
			.flatMap(
				queryStep -> queryStep.getQualifiedSelects().getSqlSelects().stream())
