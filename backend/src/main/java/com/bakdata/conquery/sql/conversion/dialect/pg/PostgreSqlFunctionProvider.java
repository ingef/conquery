...
public class PostgreSqlFunctionProvider implements SqlFunctionProvider {
    ...
    public Collection<? extends OrderField<?>> orderByValidityDates(Function<Field<?>, ? extends SortField<?>> ordering, List<Field<?>> validityDateFields) {
        ...
        return validityDateFields.stream().map(field -> nullif(field, emptyDateRange())).map(ordering).map(SortField::nullsLast).toList();
    }
    ...
    public Condition dateRestriction(ColumnDateRange dateRestriction, ColumnDateRange daterange) {
        ...
        return condition("{0} && {1}", ensureIsSingleColumnRange(dateRestriction).getRange(), ensureIsSingleColumnRange(daterange).getRange());
    }

    public Field<?> daterange(Field<?> startColumn, Field<?> endColumn, String bounds) {
        return function("daterange", Object.class, startColumn, endColumn, inline(bounds));
    }
    ...
    public List<ColumnDateRange> forCDateSet(CDateSet dateset, SharedAliases alias) {
        ...
        Field[] daterangeFields = dateset.asRanges().stream().map(this::forCDateRange).map(ColumnDateRange::getRange).toArray(Field[]::new);
    ...
    private ColumnDateRange ofSingleColumn(String tableName, Column column) {
        ...
        dateRange = switch (column.getType()) {
            ...
            case DATE_RANGE -> {
                ...
                yield when(daterange.isNull(), emptyDateRange()).otherwise(daterange(withOpenLowerEnd, withOpenUpperEnd, OPEN_RANGE));
            }
            ...
            case DATE -> {
                ...
                yield when(singleDate.isNull(), emptyDateRange()).otherwise(daterange(withOpenLowerEnd, withOpenUpperEnd, CLOSED_RANGE));
            }
            default ->
                    throw new IllegalArgumentException("Given column type '%s' can't be converted to a proper date restriction.".formatted(column.getType()));
        };
    ...
    public ColumnDateRange allRangeIf(Condition condition) {
        return ColumnDateRange.of(when(condition.isTrue(), datemultirange(daterange(toDateField(NEGATIVE_INFINITY_DATE_VALUE), toDateField(INFINITY_DATE_VALUE), CLOSED_RANGE))));
    }
    ...
    public ColumnDateRange intersection(ColumnDateRange left, ColumnDateRange right) {
        return ColumnDateRange.of(field("{0} * {1}", ensureIsSingleColumnRange(right).getRange(), ensureIsSingleColumnRange(left).getRange()));
    }
    ...
    public QueryStep unnestDaterange(ColumnDateRange nested, QueryStep predecessor, String cteName) {
        ...
        Selects selects = Selects.builder().ids(predecessor.getQualifiedSelects().getIds()).validityDate(Optional.of(unnested)).build();

        return QueryStep.builder().cteName(cteName).selects(selects).fromTable(QueryStep.toTableLike(predecessor.getCteName())).build();
    }
    ...
    public Field<Integer> extract(DatePart datePart, Field<?> timeInterval) {
        return field("{0}({1} {2} {3})", Integer.class, keyword("extract"), keyword(datePart.toSQL()), keyword("from"), timeInterval);
    }
    ...
    public <T> Field<T> random(Field<T> column) {
        ArrayAggOrderByStep<Object[]> arrayAgg = arrayAgg(field("{0} {1} {2}", column, keyword("ORDER BY"), function("random", Object.class)));
    ...
    public Field<String> yearQuarter(Field<Date> dateField) {
        return field("{0}::varchar || '-Q' || {1}::varchar", String.class, DSL.extract(dateField, DatePart.YEAR), DSL.extract(dateField, DatePart.QUARTER));
    }
    ...
    private ColumnDateRange ensureIsSingleColumnRange(ColumnDateRange daterange) {
        return daterange.isSingleColumnRange() ? daterange : ColumnDateRange.of(daterange(daterange.getStart(), daterange.getEnd(), OPEN_RANGE)); // end is already exclusive
    }
