...
public class DistinctSelectConverter implements SelectConverter<DistinctSelect> {
	...
	private enum DistinctSelectCteStep implements CteStep {
		...
		DISTINCT_SELECT("distinct", null), STRING_AGG("aggregated", DISTINCT_SELECT);
	...
	public ConnectorSqlSelects connectorSelect(
		...
		SingleColumnSqlSelect finalSelect = preprocessingSelect.qualify(
			tables.cteName(ConceptCteStep.AGGREGATION_FILTER));

		return ConnectorSqlSelects.builder()
			.preprocessingSelect(preprocessingSelect)
			.additionalPredecessor(
				Optional.of(aggregatedCte))
	...
	private static QueryStep createAggregationCte(
		...
		Field<String> castedColumn = functionProvider.cast(
			preprocessingSelect.qualify(distinctSelectCte.getCteName()).select(),
			SQLDataType.VARCHAR);
		Field<String> aggregatedColumn = functionProvider.stringAggregation(
			castedColumn,
			DSL.toChar(ResultSetProcessor.UNIT_SEPARATOR),
			List.of(castedColumn)).as(alias);

		SqlIdColumns ids = distinctSelectCte.getQualifiedSelects().getIds();

		Selects selects = Selects.builder().ids(ids).sqlSelect(new FieldWrapper<>(aggregatedColumn)).build();

		return QueryStep.builder()
			.cteName(
				selectContext.getNameGenerator().cteStepName(DistinctSelectCteStep.STRING_AGG, alias))
			.selects(
				selects)
	...
	private static QueryStep createDistinctSelectCte(
		...
		Selects selects = Selects.builder().ids(ids).sqlSelect(qualified).build();

		return QueryStep.builder()
			.cteName(
				selectContext.getNameGenerator().cteStepName(DistinctSelectCteStep.DISTINCT_SELECT, alias))
			.selectDistinct(
				true)
			.selects(selects)
