...
public class ConceptColumnSelectConverter implements SelectConverter<ConceptColumnSelect> {
	...
	private enum CONCEPT_COLUMN_STEPS implements CteStep {
		...
		UNIONED_COLUMNS("unioned_columns"), STRING_AGG("concept_column_aggregated");
	...
	public ConnectorSqlSelects connectorSelect(
		...
		ExtractingSqlSelect<Object> connectorColumn = new ExtractingSqlSelect<>(
			connector.resolveTableId().getTable(),
			connector.getColumn().getColumn(),
			Object.class);
		return ConnectorSqlSelects.builder().preprocessingSelect(connectorColumn).build();
	}
	...
	public ConceptSqlSelects conceptSelect(ConceptColumnSelect select, SelectContext<ConceptSqlTables> selectContext) {
		...
		if (isSingleConnector(selectContext.getTables())) {
			...
			connectors = selectContext.getTables()
				.getConnectorTables()
				.stream()
				.map(
					ConnectorSqlTables::getConnector)
				.toList();
		}
		...
		Selects selects = Selects.builder().ids(unionStepSelects.getIds()).sqlSelect(conceptColumnSelect).build();
		...
		QueryStep stringAggStep = QueryStep.builder()
			...
			.selects(selects)
			.fromTable(
				QueryStep.toTableLike(unionStep.getCteName()))
	...
	private static QueryStep createUnionConnectorConnectorsStep(
		...
		List<QueryStep> unionSteps = connectors.stream()
			.map(
				connector -> createConnectorColumnSelectQuery(connector, alias, selectContext))
			.toList();
		String unionedColumnsCteName = selectContext.getNameGenerator()
			.cteStepName(
				CONCEPT_COLUMN_STEPS.UNIONED_COLUMNS,
				alias);
		return QueryStep.createUnionStep(
			unionSteps,
			unionedColumnsCteName,
			Collections.emptyList(),
			false,
			selectContext.getFunctionProvider()); //TODO is false correct here?
	}
	...
	private static QueryStep createConnectorColumnSelectQuery(
		...
		String tableName = selectContext.getTables()
			...
			.stream()
			.filter(
				tables -> Objects.equals(tables.getRootTable(), connector.resolveTableId().getTable()))
			.findFirst()
			.map(
				tables -> tables.cteName(ConceptCteStep.PREPROCESSING))
		...
		Field<Object> connectorColumn = DSL.field(
			DSL.name(connectorTable.getName(), connector.getColumn().resolve().getName()));
		...
		Selects selects = Selects.builder().ids(ids).sqlSelect(connectorSelect).build();

		return QueryStep.builder().selects(selects).fromTable(connectorTable).build();
	}

	private static FieldWrapper<String> createConnectorColumnStringAgg(
		SelectContext<ConceptSqlTables> selectContext,
		QueryStep unionStep,
		String alias) {
		...
		return new FieldWrapper<>(
			functionProvider.stringAggregation(
				unionedColumn,
				DSL.toChar(ResultSetProcessor.UNIT_SEPARATOR),
				List.of(unionedColumn)).as(alias)
		);
	}
