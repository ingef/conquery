...
public class MultiSelectCondition implements WhereCondition {
	...
	public Condition condition() {
		...
		if (valuesWithoutNull.length == 0) {
			inCondition = noCondition();
		} else if (valuesWithoutNull.length == 1) {
			inCondition = field(column).eq(val(valuesWithoutNull[0]));
		} else {
			inCondition = functionProvider.in(column, valuesWithoutNull);
		}

		Condition emptyCondition;

		if (Arrays.stream(values).anyMatch(Strings::isNullOrEmpty)) {
			emptyCondition = field(column).isNull();
		} else {
			emptyCondition = noCondition();
		}

		return inCondition.or(emptyCondition);
	}
