...
public class EventDurationSumSelectConverter implements SelectConverter<EventDurationSumSelect> {
	...
	@Override
	public ConceptSqlSelects conceptSelect(
		EventDurationSumSelect select,
	...
	private FieldWrapper<BigDecimal> createEventDurationSumAggregation(
		EventDurationSumSelect select,
		...
		Field<BigDecimal> durationSum = DSL.sum(
			DSL.when(containsInfinityDate(asDualColumn, functionProvider), DSL.inline(null, Integer.class))
				.otherwise(
					functionProvider.dateDistance(ChronoUnit.DAYS, asDualColumn.getStart(), asDualColumn.getEnd()))
		).as(alias);
