...
class PreprocessingCte extends ConnectorCte {
	...
	public static QueryStep.QueryStepBuilder joinWithStratificationTable(
		...
		SqlIdColumns rootTableIds = tableContext.getIds().getPredecessor()
												.orElseThrow(() -> new IllegalStateException("Id's should have been qualified during conversion and thus have a predecessor"));
	...
	public QueryStep.QueryStepBuilder convertStep(CQTableContext tableContext) {
		...
		if (!tableContext.getConversionContext().isWithStratification()) {
			TableLike<Record> rootTable = QueryStep.toTable(tableContext.getConnectorTables().getPredecessor(ConceptCteStep.PREPROCESSING));
