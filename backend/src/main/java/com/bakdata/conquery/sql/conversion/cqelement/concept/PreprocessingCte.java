...
class PreprocessingCte extends ConnectorCte {
	...
	public QueryStep.QueryStepBuilder convertStep(CQTableContext tableContext) {
		...
		List<SqlSelect> forPreprocessing = tableContext.allSqlSelects()
			.stream()
			.flatMap(
				sqlSelects -> sqlSelects.getPreprocessingSelects().stream())
			.toList();

		Selects preprocessingSelects = Selects.builder()
			.ids(tableContext.getIds())
			.validityDate(
				Optional.of(tableContext.getValidityDate()))
		...
		tableContext.getIds()
			...
			.ifPresent(
				secondaryId -> conditions.add(secondaryId.isNotNull()));

		QueryStep.QueryStepBuilder builder = QueryStep.builder().selects(preprocessingSelects).conditions(conditions);
		...
		TableLike<Record> rootTable = QueryStep.toTableLike(
	...
	private static QueryStep.QueryStepBuilder joinWithStratificationTable(
		...
		ColumnDateRange stratificationDate = stratificationSelects.getStratificationDate()
			.orElseThrow(
				() -> new IllegalStateException(
		...
		Table<Record> connectorTable = DSL.table(
			DSL.name(tableContext.getConnectorTables().getPredecessor(ConceptCteStep.PREPROCESSING)));
		...
		Selects selects = Selects.builder()
			...
			.validityDate(
				Optional.of(tableContext.getValidityDate()))
			.stratificationDate(
				stratificationSelects.getStratificationDate())
			.sqlSelects(preprocessingSelects)
			.build();

		return QueryStep.builder().selects(selects).fromTable(joinedTable).conditions(conditions);
	}
