...
public class DateDistanceSqlAggregator implements SqlAggregator {
	...
	private DateDistanceSqlAggregator(
		...
		if (filterValue == null) {
			...
			Field<Integer> qualifiedDateDistance = dateDistanceSelect.createAliasedReference(conceptTables.getPredecessor(ConnectorCteStep.AGGREGATION_SELECT))
																	 .select();
			FieldWrapper<Integer> minDateDistance = new FieldWrapper<>(DSL.min(qualifiedDateDistance).as(alias));

			ExtractingSqlSelect<Integer> finalSelect = minDateDistance.createAliasedReference(conceptTables.getPredecessor(ConnectorCteStep.FINAL));
			...
			Field<Integer>
					...
					dateDistanceSelect.createAliasedReference(conceptTables.getPredecessor(ConnectorCteStep.EVENT_FILTER)).select();
