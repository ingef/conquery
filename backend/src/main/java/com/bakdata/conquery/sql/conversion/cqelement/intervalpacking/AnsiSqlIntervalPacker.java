...
public class AnsiSqlIntervalPacker implements IntervalPacker {
	...
	private QueryStep createPreviousEndStep(IntervalPackingContext context, AggregationMode aggregationMode) {
		...
		Field<Date> previousEnd = DSL.max(daterange.getEnd())
			.over(
				DSL.partitionBy(ids.toFields())
					...
					.andPreceding(1))
			.as(
				IntervalPacker.PREVIOUS_END_FIELD_NAME);

		List<SqlSelect> qualifiedSelects = new ArrayList<>(
			QualifyingUtil.qualify(context.getCarryThroughSelects(), sourceTableName));
		qualifiedSelects.add(
			new FieldWrapper<>(previousEnd, daterange.getStart().getName(), daterange.getEnd().getName()));
		...
		return QueryStep.builder()
			...
			.predecessors(
				Optional.ofNullable(context.getPredecessor()).stream().toList())
			.build();
	}

	private QueryStep createRangeIndexStep(
		QueryStep previousEndStep,
		IntervalPackingContext context,
		...
		Field<Date> previousEnd = DSL.field(
			DSL.name(previousEndCteName, IntervalPacker.PREVIOUS_END_FIELD_NAME),
			Date.class);

		Field<BigDecimal> rangeIndex = DSL.sum(
		...
		List<SqlSelect> qualifiedSelects = new ArrayList<>(
			QualifyingUtil.qualify(context.getCarryThroughSelects(), previousEndCteName));
		...
		return QueryStep.builder()
			...
			.predecessors(
				List.of(previousEndStep))
			.build();
	}

	private QueryStep createIntervalCompleteStep(
		QueryStep rangeIndexStep,
		IntervalPackingContext context,
		AggregationMode aggregationMode) {
		...
		Field<BigDecimal> rangeIndex = DSL.field(
			DSL.name(rangeIndexCteName, IntervalPacker.RANGE_INDEX_FIELD_NAME),
			BigDecimal.class);
		...
		return QueryStep.builder()
			.cteName(
				context.getTables().cteName(IntervalPackingCteStep.INTERVAL_COMPLETE))
			.selects(
				intervalCompleteSelects)
			.fromTable(QueryStep.toTableLike(rangeIndexCteName))
			.predecessors(
				List.of(rangeIndexStep))
	...
	private static Selects buildSelects(
		...
		Selects.SelectsBuilder builder = Selects.builder().ids(ids);
